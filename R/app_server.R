...
app_server <- function(input, output, session) {
  ...
  # Reactive list for module communication ----
  r <- shiny::reactiveValues()

  # Module calls ---
  mod_upload_server("upload_1", r = r)
  mod_filter_server("server_1", r = r)


  ####    3. dendrogram output ####
  output$dendro_1 <- shiny::renderPlot({
    shiny::req(
      prepare_dist_matrix_for_clustering(),
      shiny::isolate(input$clusterMethod)
    )
    if (
  ...
  output$hover <- shiny::renderPlot(
    {
      ...
      shiny::req(
        shiny::isolate(data_with_selected_factor_levels()),
      ...
      if (
        ...
        if (plot_coords$coords_css$y > 0 & plot_coords$coords_css$x > 0) {
          ...
          if (!is.null(y) && !is.null(x)) {
            ...
            dat_filt <- dat[
              dat$TRTP == levels(dat$TRTP)[ceiling(y / input$zoompx)] &
                dat$LBTESTCD == sortin[ceiling(x / input$zoompx)],
            ]
            ...
            if (input$con_lin) {
              lines_data <- quant_plot_data_lines() %>%
                dplyr::filter(
                  ...
                  LBTESTCD == sortin[ceiling(x / isolate(input$zoompx))]
                )
            } else {
              lines_data <- NULL
            }
  ...
  output$hoverpanel <- shiny::renderUI({
    shiny::absolutePanel(
      ...
      HTML(
        '<button style = "background: #f6ad82; color:#ffffff", data-toggle="collapse" data-target="#demo" style="color:white;"> <i class="fa-solid fa-search-plus"></i> Open/Close Zoom Panel</button>'
      ),
      top = 70,
      left = "auto",
      right = 100,
      ...
      tags$div(
        ...
        class = "collapse",
        shiny::fluidRow(
          shiny::column(2, shiny::plotOutput('hover'))
        ),
        shiny::fluidRow(
          shiny::column(
            12,
        ...
        shiny::fluidRow(
          shiny::column(12, uiOutput('hover_info_text'))
        )
      ),
      style = "z-index: 99999;"
    )
  ...
  output$hover_info_text <- shiny::renderUI({
    input$apply_quant_plot
    shiny::req(
      shiny::isolate(data_with_missing_flag()),
      input$plot_option_switch
    )
  ...
  output$legend <- shiny::renderPlot(
    {
      ...
      graphics::rect(
        ...
        col = c(c(
          'white',
          colChoice[[shiny::req(input$select.pal1)]]$col,
          'black'
        )),
        border = TRUE
      )
  ...
  output$legendpanel <- shiny::renderUI({
    shiny::absolutePanel(
      ...
      HTML(
        '<button style="background: #f6ad82; color:#ffffff", data-toggle="collapse" data-target="#demo_co">Open/Close</button>'
      ),
      tags$div(
        id = 'demo_co',
        class = "collapse in",
  ...
  output$hoverpanel2 <- shiny::renderUI({
    ...
    shiny::absolutePanel(
      ...
      HTML(paste0("<div style='background-color:", ColorBG, "'>")),
      HTML(
        '<button style="background: #f6ad82; color:#ffffff", data-toggle="collapse" data-target="#demo2"> <i class="fa-solid fa-search-plus"></i> Open/Close Zoom Panel</button>'
      ),
      top = 70,
      left = "auto",
      right = 100,
      ...
      tags$div(
        ...
        class = "collapse",
        shiny::fluidRow(
          shiny::column(2, shiny::plotOutput('hover2'))
        ),
        shiny::fluidRow(
          shiny::column(
            12,
        ...
        shiny::fluidRow(
          shiny::column(12, uiOutput('hover_info_text2'))
        )
      ),
      style = "z-index: 10;"
    )
    ...
    output$hover2 <- shiny::renderPlot(
      {
        ...
        shiny::req(
          ...
          Summary_for_qualitative_trends(),
            ...
            dat_filt <- dat[
              dat$TRTP ==
                levels(dat$TRTP)[ceiling(
                  plot_coords$coords_css$y / input$zoompx
                )] &
                dat$LBTESTCD ==
  ...
  output$hoverpanel3 <- shiny::renderUI({
    shiny::absolutePanel(
      ...
      HTML(paste0("<div style='background-color:", ColorBG, "'>")),
      HTML(
        '<button style="background: #f6ad82; color:#ffffff", data-toggle="collapse" data-target="#demo3"> <i class="fa-solid fa-search-plus"></i> Open/Close Zoom Panel</button>'
      ),
      top = 70,
      left = "auto",
      right = 100,
      ...
      tags$div(
        ...
        class = "collapse",
        shiny::fluidRow(
          shiny::column(2, shiny::plotOutput('hover3'))
        ),
        shiny::fluidRow(
          shiny::column(
            12,
        ...
        shiny::fluidRow(
          shiny::column(12, uiOutput('hover_info_text3'))
        )
      ),
      style = "z-index: 10;"
    )
        ...
        if (
          ...
          dat_filt <- dat[
            dat$TRTP ==
              ...
              dat$LBTESTCD ==
                sorti[ceiling(plot_coords$coords_css$x / input$zoompx)],
          ]
          ...
          elaborator_plot_ref_pattern(
            ...
            sorting_vector = sorti[ceiling(
              plot_coords$coords_css$x / input$zoompx
            )],
      ...
      output$trendPlot <- shiny::renderPlot(
        {
          ...
          elaborator_plot_qual_trends(
            ...
            color_palette = c(
              'white',
              colChoice[[shiny::req(isolate(input$select.pal1))]]$col,
              'black'
            ),
            Summa = Summa
          )
      ...
      output$tab2 <- shiny::renderUI({
        ...
        shiny::wellPanel(
          ...
          shiny::plotOutput(
            ...
            click = clickOpts("dist_click2", clip = FALSE)
          )
        )
      })
    }
  ...
  shiny::observeEvent(input$select.lab, {
    if (length(input$select.lab) <= length(input$arrange.lab)) {
      ...
      tmp <- c(
        input$arrange.lab,
        input$select.lab[!input$select.lab %in% input$arrange.lab]
      )
    }
          ...
          shiny::column(
            ...
            shiny::wellPanel(
              ...
              shiny::plotOutput(
                ...
                click = clickOpts("dist_click", clip = FALSE)
              )
            )
          )
        )
  ...
  output$cont1 <- shiny::renderUI({
    list(
      shiny::tags$head(
        tags$style(HTML(
          '#apply_quant_plot{color: #ffffff; background-color:#47d2bc;
          border-color: #f78300}'
        ))
      )
    )
  ...
  observeEvent(input$apply_quant_plot, {
    output$cont1 <- shiny::renderUI({
      list(
        shiny::tags$head(
          tags$style(HTML(
            '#apply_quant_plot{color: #ffffff; background-color:#e3e3e3;
            border-color: #ffffff}'
          ))
        )
      )
  ...
  output$cont2 <- shiny::renderUI({
    list(
      shiny::tags$head(
        tags$style(HTML(
          '#apply_qual_plot{color: #ffffff; background-color:#47d2bc;
          border-color: #f78300}'
        ))
      )
    )
  ...
  observeEvent(input$apply_qual_plot, {
    output$cont2 <- shiny::renderUI({
      list(
        shiny::tags$head(
          tags$style(HTML(
            '#apply_qual_plot{color: #ffffff; background-color:#e3e3e3;
            border-color: #ffffff}'
          ))
        )
      )
  ...
  output$cont3 <- shiny::renderUI({
    list(
      shiny::tags$head(
        tags$style(HTML(
          '#apply_ref_plot{color: #ffffff; background-color:#47d2bc;
          border-color: #f78300}'
        ))
      )
    )
  ...
  observeEvent(input$apply_ref_plot, {
    output$cont3 <- shiny::renderUI({
      list(
        shiny::tags$head(
          tags$style(HTML(
            '#apply_ref_plot{color: #ffffff; background-color:#e3e3e3;
            border-color: #ffffff}'
          ))
        )
      )
